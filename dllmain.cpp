...
extern "C" __declspec(dllexport) void* get_init_addr(char* arcversion, void* imguictx, void* id3dptr, HMODULE  arcdll, void* mallocfn, void* freefn, uint32_t d3dversion) {
...
void squad_update_callback(const UserInfo* updatedUsers, size_t updatedUsersCount) {
	...
	for (size_t i = 0; i < updatedUsersCount; i++) {
		...
		log_debug(std::format("updated user {} accountname: {} ready: {} role: {} jointime: {} subgroup: {}",
			i,
			user.AccountName,
			user.ReadyStatus,
			(uint8_t)user.Role,
...
extern "C" __declspec(dllexport) void arcdps_unofficial_extras_subscriber_init(
	...
	if (initFailed) {
		log_squad("init failed, not continuing with arcdps_unofficial_extras_subscriber_init");
		return;
	}
